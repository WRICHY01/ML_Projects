...
class RouteToLimited(BaseModel):
    """
    Route a user query to the most relevant data source["llm_knowledge", "web_search"].
    """
...
SOURCES = """
                            - llm_knowledge: General reasoning, logic, creativity, coding help, explanations, and stable background knowledge (knowledge cutoff: January 2025)
...
SYSTEM_INSTRUCTIONS_RTL = f"""
                            ...
                            **Tie-breaker**: If the question has both analytical and real-time components, choose web_search.

                            EXAMPLES:
                            - "Explain quantum computing" → llm_knowledge
                            - "What's the weather today?" → web_search
                            - "Write a poem about space" → llm_knowledge
                            - "Who is the current CEO of Tesla?" → web_search
                            - "Debug this Python code" → llm_knowledge
                            - "What are the latest developments in AI?" → web_search
                        """

SYSTEM_INSTRUCTIONS_RTA = f"""
                            You are a routing expert. Analyze the user's question and select the best data source.
...
def get_route_chain(llm, should_route_to_llm_or_web: bool):

    schema = RouteToLimited if should_route_to_llm_or_web else RouteToAll
    system_prompt = SYSTEM_INSTRUCTIONS_RTL if should_route_to_llm_or_web else SYSTEM_INSTRUCTIONS_RTA

    structured_llm_router = llm.with_structured_output(schema)
    ...
    return question_router_chain

# structured_llm_router = llm.with_structured_output(RouteToAll)
# question_router_prompt_template = ChatPromptTemplate.from_messages(
#     [
#         ("system", SYSTEM_INSTRUCTIONS_RTA),
#         ("human", "{question}")
#     ]
# )
# question_router_chain: RunnableSequence = question_router_prompt_template | structured_llm_router
