...
class QueryRouter(BaseModel):
    ...
    """
    data_source: Literal["vector_store", "llm_knowledge", "web_search", "undetermined"] = Field(
            description=(
                ...
                "'undetermined' for ambiguous, vague, or invalid queries that cannot be routed."
            )
        )
...
SYSTEM_INSTRUCTIONS = """
                            ...
                            - undetermined: Fallback for ambiguous, vague, invalid or intelligible queries.
...
# print(f"question router prompt template looks like: {question_router_prompt_template}")
# question_router_chain.invoke({"question": "what exactly is the buzz about AI agents?"})
# print(f"question router chain looks like: {question_router_chain}")